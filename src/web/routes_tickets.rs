...
async fn create_ticket(State(mc): State<ModelController>, Json(create_model): Json<CreateTicketModel>) -> Result<Json<Ticket>> {
    println!("--> {:<12} - create_ticket", "HANDLER");
    let ticket = mc.create_ticket(create_model).await?;
    Ok(Json(ticket))
}
